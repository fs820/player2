...
void InitResult(void)
{
	...
	switch (GetBossNumber())
	{
	case 0:
		if (GetClear())
		{
			PlayVideo(hWnd, MOVIE_WIN0);
		}
		else
		{
			PlayVideo(hWnd, MOVIE_LOSE0);
		}
		break;
	case 1:
		if (GetClear())
		{
			PlayVideo(hWnd, MOVIE_WIN1);
		}
		else
		{
			PlayVideo(hWnd, MOVIE_LOSE1);
		}
		break;
	case 2:
		if (GetClear())
		{
			PlayVideo(hWnd, MOVIE_WIN2);
		}
		else
		{
			PlayVideo(hWnd, MOVIE_LOSE2);
		}
		break;
	case 3:
		if (GetClear())
		{
			PlayVideo(hWnd, MOVIE_WIN3);
		}
		else
		{
			PlayVideo(hWnd, MOVIE_LOSE3);
		}
		break;
	}
	...
	(
		sizeof(VERTEX_2D) * VT_MAX*(SCORE_MAX+2),
		D3DUSAGE_WRITEONLY,
		FVF_VERTEX_2D,
		D3DPOOL_MANAGED,
		&g_pVtxBuffResult,
	...
	//座標設定
	pVtx[0].pos = D3DXVECTOR3(SCREEN_WIDTH - RESULT_WIDTH, 0.0f, 0.0f);
	pVtx[1].pos = D3DXVECTOR3(SCREEN_WIDTH, 0.0f, 0.0f);
	pVtx[2].pos = D3DXVECTOR3(SCREEN_WIDTH - RESULT_WIDTH, RESULT_HEIGHT, 0.0f);
	pVtx[3].pos = D3DXVECTOR3(SCREEN_WIDTH, RESULT_HEIGHT, 0.0f);
...
void DrawResult(void)
{
	...
	{
		//テクスチャの設定
		pDevice->SetTexture(0, g_apTextureResult[2]);
	}
	else
	{
		//テクスチャの設定
		pDevice->SetTexture(0, g_apTextureResult[3]);
	}
